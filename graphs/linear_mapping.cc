...
Operation::Operation(string cigar, string query, string reference) {
...
void Operation::DecodeOperation(char type_encoding, int length, string query,
  ...
  switch (type_encoding) {
    ...
    case 'S':
      ...
      break;
    default:
      throw logic_error(to_string(type_encoding) +
...
void Operation::Validate() const {
  ...
  throw logic_error(query_ + " and " + reference_ +
...
string Operation::GetCigarString() const {
...
Mapping::Mapping(int32_t reference_start, const string& cigar,
...
void Mapping::DecodeOperations(int32_t reference_start, const string& cigar,
                               const string& query, const string& reference) {
    ...
    if (isalpha(c) != 0) {
      ...
      switch (c) {
        ...
        case 'D':
          ...
          break;
        default:
          throw logic_error(to_string(c) + " is unknown CIGAR operation");
      }
