    ...
    vector<std::unique_ptr<RegionAnalyzer>>& locusAnalyzers, int searchRadius)
{
    ...
    {
        const RegionSpec& locusSpec = locusAnalyzer->regionSpec();

        auto extendedReferenceLocus = locusSpec.referenceRegion().Extend(searchRadius);

        LocusTypeAndAnalyzer payload(LocusType::kTargetLocus, locusAnalyzer.get());
        contigToIntervals[extendedReferenceLocus.chrom()].emplace_back(
            extendedReferenceLocus.start(), extendedReferenceLocus.end(), payload);

        for (const auto& offtargetLocus : locusSpec.offtargetRegions())
        {
