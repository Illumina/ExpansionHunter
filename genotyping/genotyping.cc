...
double
StrGenotype::calcLogLik(const map<int, int> &flanking_size_counts,
                        const map<int, int> &spanning_size_counts) const {
    ...
    for (int num_units_hap1 : haplotype_candidates) {
      for (int num_units_hap2 : haplotype_candidates) {
        ...
        StrGenotype genotype(max_num_units_in_read, prop_correct_molecules,
                             ...
                             num_units_hap2);
        const double cur_loglik =
            genotype.calcLogLik(flanking_size_count, spanning_size_count);
