...
public:
    ...
    MOCK_METHOD0(GetRead, reads::Read::SharedPtr(void));
};
...
TEST(ReadExtraction, TypicalRegion_ReadsExtracted)
{
    ...
    Read::SharedPtr read1 = std::make_shared<Read>("frag1", "ATCG");
    read1->SetIsFirstMate(true);
    Read::SharedPtr read2 = std::make_shared<Read>("frag1", "GCTA");
    read2->SetIsFirstMate(false);

    EXPECT_CALL(mock_reader, SetRegion(target_region));
    EXPECT_CALL(mock_reader, GetRead()).WillOnce(Return(read1)).WillOnce(Return(read2)).WillOnce(Return(nullptr));
