...
bool LoadFlanks(const string &genome_path, double min_wp,
  ...
  const string left_flank_coords = repeat_region.chrom() + ":" +
                                   to_string(left_flank_begin) + "-" +
                                   to_string(left_flank_end);
  const string right_flank_coords = repeat_region.chrom() + ":" +
                                    to_string(right_flank_begin) + "-" +
                                    to_string(right_flank_end);
  ...
  const string repeat_coords = repeat_region.chrom() + ":" +
                               to_string(left_flank_end + 1) + "-" +
                               to_string(right_flank_begin - 1);
  ...
  if (ref_repeat_wp < min_wp) {
    spd::get("console")->warn(
        "reference sequence of {} ({}) has low WP score of {}",
        repeat_spec->repeat_id, repeat_spec->ref_seq, to_string(ref_repeat_wp));
  }

  return true;
}

void LoadRepeatSpecs(const string &specs_path, const string &genome_path,
                     double min_wp, map<string, RepeatSpec> &repeat_specs) {
  const boost::regex regex_json(".*\\.json$");

  boost::smatch what;
  const bool is_path_to_json = boost::regex_match(specs_path, what, regex_json);

  if (is_path_to_json) {
    spd::get("console")->info("Loading specs from JSON file {}", specs_path);
    ...
    spd::get("console")->info("Loading specs from directory {}", specs_path);
    ...
    for (boost::filesystem::directory_iterator itr(path); itr != end_itr;
      ...
      if (is_regular_file(itr->status())) {
        ...
        if (boost::regex_match(fname, what, regex_json)) {
          spd::get("console")->info("Loading {}", fname);
